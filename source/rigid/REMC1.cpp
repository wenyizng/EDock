...
bool REMC(AMBER_TYPER & amber, vector <DOCKMol> & REmolvec,Energy_Score & c_nrg, string filevdw, string fileflex,string fileflex_drive_tbl,string grid_file,int cutoff,vector<vector <float> > &x,vector<vector <float> > &y, vector<vector <float> > &z,vector<float> &accenergy,float Tmin, float Tmax,string outfile,mt19937 &gen, vector<TORSION> &torsions, bool flexible_flag,vector<float>sphx,vector<float>sphy,vector<float> sphz)
{
	...
	cout<<i<<"\t"<<temperature[i-1]<<"\t"<<REmolvec[i-1].current_score<<endl;
	...
	for(int i=0;i<Tvecmol.size();i++)
	{
	    ...
	    //if(MC(amber, Tvecmol[i], c_nrg,filevdw.c_str(), fileflex.c_str(),fileflex_drive_tbl.c_str(),grid_file,1,tmpx,tmpy,tmpz, tmpenergy,temperature[i],Rotationangle[i],gen,MCsteps))   
             if(MC(amber, Tvecmol[i], c_nrg,filevdw.c_str(), fileflex.c_str(),fileflex_drive_tbl.c_str(),grid_file,1,tmpx,tmpy,tmpz, tmpenergy,temperature[i],gen,replica_MC_energy[i],torsions,flexible_flag,sphx,sphy,sphz))
	    {
		//selection 1
	...
	for(int i=startindex;i<Tvecmol.size()-1;i=i+2)
	{
		...
		//for output
                ...
                if(replica_MC_energy[i].size()==0 && replica_MC_energy[i+1].size()!=0)
                {
                    ...
                    replica_MC_energy[i+1][replica_MC_energy[i+1].size()-1]=1e+5;
                    continue;
                }
                if(replica_MC_energy[i].size()!=0 && replica_MC_energy[i+1].size()==0)
                {
                    replica_MC_energy[i][replica_MC_energy[i].size()-1]=1e+5;
                    continue;
                }
	...
	cout<<i<<" replica size:"<<Tvecenergy[i].size()<<" total energy:"<<tempenergy<<endl;
	...
	swapacc[i]=swaptimes[i]*1.0/(swapnum);
        conformationsacc[i] = Tvecenergy[i].size()*1.0/(swapnum*500);
        if(Tvecenergy[i].size()!=0)
        {
            cout<<i<<"\t"<<temperature[i]<<"\t"<<swaptimes[i]<<"\t"<<swapnum<<"\t"<<swapacc[i]<<"\t"<<Tvecenergy[i].size()<<"\t"<<conformationsacc[i]<<"\t"<<Tvecenergy[i][Tvecenergy[i].size()-1]<<"\t"<<ave_energy[i]<<endl;
            fpacc<<i<<"\t"<<temperature[i]<<"\t"<<swaptimes[i]<<"\t"<<swapacc[i]<<"\t"<<Tvecenergy[i].size()<<"\t"<<conformationsacc[i]<<"\t"<<Tvecenergy[i][Tvecenergy[i].size()-1]<<"\t"<<ave_energy[i]<<endl;
        }
        if(Tvecenergy[i].size()==0)
        {
            cout<<i<<"\t"<<temperature[i]<<"\t"<<swaptimes[i]<<"\t"<<swapnum<<"\t"<<swapacc[i]<<"\t"<<Tvecenergy[i].size()<<"\t"<<conformationsacc[i]<<"\t"<<"nan"<<"\t"<<"nan"<<endl;
            fpacc<<i<<"\t"<<temperature[i]<<"\t"<<swaptimes[i]<<"\t"<<swapnum<<"\t"<<swapacc[i]<<"\t"<<Tvecenergy[i].size()<<"\t"<<conformationsacc[i]<<"\t"<<"nan"<<"\t"<<"nan"<<endl;
        }
